...
const AcceptCredentials = (props) => {
    ...
    const saveCredential = async (credential) => {
        try {
          console.log("saveCredentials credential : ", credential);
            const {credentialIds} = await storeSignedVCs({
                data: [credential]
              });
            console.log("SaveCredential AcceptCredentials: ",credentialIds, credentialIds[0]);
    ...
    useEffect(() => {
      ...
      try {
        checkLogin()
        if (queryString.parse(props.location.search).vcURL) {
          console.log("VC URL Holder AcceptCredentials: ", queryString.parse(props.location.search).vcURL);
